import { PhoneOutlined, MailOutlined, EnvironmentOutlined } from '@ant-design/icons';

const ContactInfo = () => {
  return (
    <div className="bg-[#F3F2F1] py-10 px-4 md:px-8">
      <div className="max-w-2xl mx-auto text-start mb-10">
        <h1 className="text-3xl font-semibold text-black mb-2">Contact Information</h1>
        <p className="text-gray-600 text-[18px]">Say something to start a live chat!</p>
      </div>

      <div className="w-full mx-auto flex flex-col gap-6">
        <div className="flex items-start gap-4">
          <PhoneOutlined className="text-2xl text-primary mt-1" />
          <div>
            <p className="text-gray-700 font-medium">09606292909</p>
            
          </div>
        </div>
        <div className="flex items-start gap-4">
          <PhoneOutlined className="text-2xl text-primary mt-1" />
          <div>
            
            <p className="text-gray-700 font-medium">01879997999</p>
          </div>
        </div>

        <div className="flex items-start gap-4">
          <MailOutlined className="text-2xl text-primary mt-1" />
          <div>
            <p className="text-gray-700 font-medium">hello@stayverz.com</p>
          </div>
        </div>

        <div className="flex items-start gap-4">
          <EnvironmentOutlined className="text-2xl text-primary mt-1" />
          <div>
            <p className="text-gray-700 font-medium">
              2nd Floor, House - 10, Road - 11, Sector - 6, <br />
              Uttara, Dhaka - 1230
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default ContactInfo;
